...
export default function ({
  ...
  const {
    ...
    data: {
      blog: { detail = {}, comments: commentsData = [] },
      user: { currentUser = {} },
    },
  } = useDva(
    {
  ...
  const handleComments = ({ content = null, parentId = null, replyId = null, root = 0 }) => {
    return dispatch({
      ...
      <div className={styles.detail}>
        ...
        <Card bordered={false}>
          <div className={`flex ${styles.user}`}>
            <div className="flex">
              ...
              <div style={{ marginLeft: 12 }}>
                ...
                <div style={{ color: '#999' }}>
                  <span>
                    <CalendarOutlined /> {detail.updatedAt}
                  </span>
                  <span style={{ marginLeft: 8 }}>
                    <EyeOutlined /> {detail?.reading}
                  </span>
                  <span style={{ marginLeft: 8 }}>
                    <LikeOutlined /> {detail?.likes}
                  </span>
                  <span style={{ marginLeft: 8 }}>
                    <MessageOutlined /> {detail?.comments}
                  </span>
                </div>
              </div>
            </div>
          ...
          {detail?.coverPathUrl && (
            <div
              style={{
                background: `url(${detail.coverPathUrl}) no-repeat center`,
            ...
            <div className="flex">
              ...
              <span>
                {detail?.tags?.map((tag, index) => (
                  <Tag color={tagColor[index % tagColor.length]} style={{ marginRight: 4 }} key={tag.id}>
                    {tag.title}
                  </Tag>
                ))}
              </span>
          ...
          <ScrollEle name="comments">
            <div className={styles.comments}>
              ...
              <Card bordered={false}>
                <CommentsBox
                  handleComments={useCallback(handleComments, [])}
        ...
        <div className={styles.extra}>
          <div onClick={handleLikes} className={isLike ? styles.likes : ''}>
            <LikeOutlined /> 点赞
          </div>
          <div onClick={scrollToComments}>
            <MessageOutlined /> 评论
          </div>
        </div>
      </div>
